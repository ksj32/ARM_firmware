...
extern unsigned int HandleTIMER0; 	 //타이머 인터럽트 핸들러 선언 
...
void Timer_Init(void)				 					/* init */
{
	...
	rTCON |= (0x1<<11);		//timer1 auto_reload	(auto_reload : 값을 새로 셋팅) 
...
void timer1_handler(void) /* timer 1 handler is associated with make Note & drop Note */
{ 
	rINTMSK1 |= BIT_TIMER1;
	rSRCPND1 |= BIT_TIMER1;
	rINTPND1 |= BIT_TIMER1;
 	//MAKE_NOTE(START_X, RANDOM, HIT_CHECK, COLOR)
	MAKE_NOTE(start_x1,random1,hit_check1,0x001f) /* call MAKE_NOTE macro & start_x1 is 480 in init */
	if(start_x1 < -SIZE_OF_BUMB) /* start_x1 was decreased in MAKE_NOTE , So if start_x1 less than 0 */
	{
		start_x1 = 480; 		
		if(level == 1)
			random1 = rand()%4;
			...
			random1 = rand()%4;
		/* if hit_check = 0, you didn't touch Nth button --> status MISS --> Score is decreased */		
		if(hit_check1 < 0)	//버튼을 누르지 않았을 경우 
			score -= 10; 	//점수 10점 하락  
		hit_check1 = 0;     //다시 초기화           
	}
	
	if(level == 1)   /* level is associated with timer2 handler */
	{
		MAKE_NOTE(start_x2,random2,hit_check2,0x7c00) /* different color Note is made */
...
void timer2_handler(void) /* timer 2 handler is associated with Increase Note & Game Over & Game Clear */
{
	rINTMSK1 |= BIT_TIMER2;
	rSRCPND1 |= BIT_TIMER2;
	rINTPND1 |= BIT_TIMER2;
